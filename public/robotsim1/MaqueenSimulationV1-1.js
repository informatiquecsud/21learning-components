class simulation {
  constructor({
    ...
    mouse = mouse || true;
    debug = debug || false;
    background = background || 0xcccac0;
    this.robots = [];
    this.walls = [];
    this.marks = [];
    this.zones = [];
  ...
  wallCircle(x, y, r) {
    return new wallCircle(this.scenes[0], x, y, r);
  }
  wallRect(x, y, width, height, angle = 0) {
  ...
  zoneCircle(x, y, radius, callback, color = 0xff0000, alpha = 0.3) {
    return new zoneCircle(this.scenes[0], x, y, radius, callback, color, alpha);
  }
  zoneRect(
    x,
    y,
  ...
  zonePoly(x, y, points, callback, color = 0xff0000, alpha = 0.3) {
    return new zonePoly(this.scenes[0], x, y, points, callback, color, alpha);
  }

  markCircle(x, y, radius) {
    return new markCircle(this.scenes[0], x, y, radius);
  }
  markRect(x, y, width, height, angle = 0) {
    return new markRect(this.scenes[0], x, y, width, height, angle);
  }
  markPoly(x, y, points) {
    return new markPoly(this.scenes[0], x, y, points);
  }

  picture(key, x, y, angle = 0, scaleX = 1, scaleY = 1) {
    return new Picture(this.scenes[0], key, x, y, angle, scaleX, scaleY);
  }
}
