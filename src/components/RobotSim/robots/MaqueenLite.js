class MaqueenLiteBody {
  constructor(that, id, x, y, angle = 0) {
    ...
    this.body = that.matter.add
      .sprite(x, y, "bodyPic", "", {
        shape: that.cache.json.get("bodyShape").body,
      })
      .setFrictionAir(0);
    this.Lwheel = that.matter.add
      .image(x - 89 * this.scale, y + 45 * this.scale, "wheel")
      .setFrictionAir(0.5);
    this.Rwheel = that.matter.add
      .image(x + 89 * this.scale, y + 45 * this.scale, "wheel")
      .setFrictionAir(0.5);
    ...
    this.ultra = that.matter.add
      .sprite(x, y - 4080 * this.scale, "ultra", "", {
        shape: that.cache.json.get("ultraShape").ultra,
      })
      .setScale((this.scale / 8) * 250)
      .setCollidesWith(0)
      .setVisible(false);
    ...
    for (let i = 0; i < that.marks.length; i++) {
      if (that.matter.overlap(this.irL, that.marks[i].body)) {
        var color = that.textures.getPixel(
          (this.irL.x -
            that.marks[i].pos.x +
            (that.marks[i].body.width * that.marks[i].scale.x) / 2) /
            that.marks[i].scale.x,
          (this.irL.y -
            that.marks[i].pos.y +
            (that.marks[i].body.width * that.marks[i].scale.y) / 2) /
  ...
  setComponent() {
    this.irL.setPosition(
      this.body.x - 18 * this.scale,
      this.body.y - 40 * this.scale
    );
    this.irR.setPosition(
      this.body.x + 18 * this.scale,
      this.body.y - 40 * this.scale
    );
...
class MaqueenLite {
  constructor(that, name, x, y, angle) {
    this.robot = new MaqueenLiteBody(that, name, x, y, angle);
    this.i2c = new MaqueenLiteI2CController(this.robot);
...
class MaqueenLiteI2CController {
  constructor(bot) {
    this.bot = bot;
  }
...
class pin {
  ...
  read_digital() {
    if (this.id == "RLed") {
      if (this.robot.state.led.right) {
        return true;
      }
    } else if (this.id == "LLed") {
      if (this.robot.state.led.left) {
        return true;
      }
    } else if (this.id == "irLeft") {
      if (this.robot.state.ir.left) {
        return true;
      }
    } else if (this.id == "irRight") {
      if (this.robot.state.ir.right) {
        return true;
      }
    }
    return false;
